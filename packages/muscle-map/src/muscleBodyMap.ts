...
/**
 * Maps FitGram-style muscle names (two vocabularies: detailed
 * primary/secondary muscles and coarse muscle groups) onto the slugs that
 * react-native-body-highlighter understands.
 */
...
/** Resolve a list of muscle names to unique body-highlighter slugs. */
 ...
 * muscle names (either vocabulary above) — resolution to slugs happens in
 * computeBodyData().
 */
...
/** Default number of intensity levels (matches the default colour ramp). */
 ...
 * intensity from 1..levels scaled against the most-worked muscle.
 */
export function computeBodyData(
  exercises: HeatmapExercise[],
  levels: number = INTENSITY_LEVELS
): BodyDatum[] {
  ...
  return [...scoreBySlug.entries()].map(([slug, score]) => ({
    slug,
    // scale to 1..levels, never 0 for a touched muscle
    intensity: Math.max(1, Math.round((score / max) * levels)),
  }));
}